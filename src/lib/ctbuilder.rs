    ...
    usize: AsPrimitive<StorageT>,
{
    ...
    use lrpar::{NonStreamingLexer, LexParseError, Lexeme, RecoveryKind, ParseError, parser::AStackType};
...
",
        ...
        match self.yacckind.unwrap() {
            YaccKind::Original(YaccOriginalActionKind::UserAction) | YaccKind::Grmtools => {
                ...
                outs.push_str(&self.gen_runtime_parser_actions(grm));
            }
    ...
    pub fn parse<'lexer, 'input: 'lexer>(
        lexer: &'lexer dyn NonStreamingLexer<'input, {lexemet}, {storaget}>{parse_param},
        grm: &YaccGrammar<u32>,
    ...
    {{",
            ...
            YaccKind::Original(YaccOriginalActionKind::GenericParseTree) => {
                outs.push_str(&format!(
                    "
    #[allow(dead_code)]
    pub fn parse(lexer: &dyn NonStreamingLexer<{lexemet}, {storaget}>,
            grm: &YaccGrammar<u32>,
            stable: &StateTable<u32>)
    ...
    {{",
       ...
       //                &'lexer dyn NonStreamingLexer<'input, {lexemet}, {storaget}>,
       //                ::lrpar::Span,
       //                ::std::vec::Drain<::lrpar::parser::AStackType<{lexemet}, {actionskind}<'input>>>,
       //                {parse_paramty}) -> {actionskind}<'input>> = ::std::vec![{wrappers}];\n",
        ...
        for pidx in grm.iter_pidxs() {
            ...
            outs.push_str(&format!(
                "    fn {prefix}wrapper_{}<'lexer, 'input: 'lexer>({prefix}ridx: ::cfgrammar::RIdx<{storaget}>,
                      {prefix}lexer: &'lexer dyn NonStreamingLexer<'input, {lexemet}, {storaget}>,
    ...
    fn gen_runtime_parser_actions(&self, grm: &YaccGrammar<StorageT>) -> String {
        let (parse_param, _) = match grm.parse_param() {
            Some((name, tyname)) => (name.to_owned(), format!("{}: {}", name, tyname)),
            None => ("()".to_owned(), "_: ()".to_owned()),
        };

                            //0 => __gt_wrapper_0(ridx, self.lexer, span, astack.drain(pop_idx - 1..), {}),
        let mut wrappers = grm
            .iter_pidxs()
            .map(|pidx| {
                format!(
                    "{pidx} => {prefix}wrapper_{pidx}(ridx, self.lexer, span, astack.drain(pop_idx - 1..), {parse_param})",
                    pidx = usize::from(pidx),
                    prefix = ACTION_PREFIX,
                    parse_param = parse_param
                )
    ...
    > {{
        ...
        pub grm: &'a YaccGrammar<u32>,
        pub token_cost: Box<TokenCostFn<'a, u32>>,
        pub stable: &'a StateTable<u32>,
    ...
    {{
                ...
                match self.stable.action(stidx, la_tidx) {{
                    Action::Reduce(pidx) => {{
                        ...
                        let span = if spans.is_empty() {{
                            ...
                            ::lrpar::Span::new(spans[pop_idx - 1].start(), spans[spans.len() - 1].end())
                        }} else {{
                            ::lrpar::Span::new(spans[spans.len() - 1].start(), spans[spans.len() - 1].end())
                        }};
                        spans.truncate(pop_idx - 1);
                        spans.push(span);

                        let act = match usize::from(pidx) {{
                            {}
                            _ => unreachable!()
                        }};
                        astack.push(AStackType::ActionType(act));
                    }}
                    ...
                    Action::Error => {{
                        match self.rcvry_kind {{
                            RecoveryKind::CPCTPlus | RecoveryKind::None => {{
                                let la_lexeme = self.next_lexeme(laidx);
                                errors.push(
                                    ParseError {{
                                        stidx,
        ...
        pub fn next_lexeme(&self, laidx: usize) -> lrlex::lexemes::DefaultLexeme {{
            ...
            if laidx < llen {{
                ...
                Lexeme::new_faulty(
                    u32::from(self.grm.eof_token_idx()),
                    last_la_end,
                    0,
                )
            }}
        ...
        /// EOF `TIdx`).
        pub fn next_tidx(&self, laidx: usize) -> TIdx<u32> {{
    ...
    pub struct RTParserBuilder<'a> {{
        grm: &'a YaccGrammar<u32>,
        stable: &'a StateTable<u32>,
        recoverer: RecoveryKind,
        term_costs: &'a dyn Fn(TIdx<u32>) -> u8,
        phantom: PhantomData<(lrlex::lexemes::DefaultLexeme, u32)>,
    }}

    impl<'a> RTParserBuilder <'a>
    {{
        /// Create a new run-time parser from a `YaccGrammar`, a `StateGraph`, and a `StateTable`.
        pub fn new(grm: &'a YaccGrammar<u32>, stable: &'a StateTable<u32>) -> Self {{
            RTParserBuilder {{
                grm,
                stable,
                recoverer: RecoveryKind::CPCTPlus,
        ...
        pub fn _term_costs(mut self, f: &'a dyn Fn(TIdx<u32>) -> u8) -> Self {{
            self.term_costs = f;
            self
        }}
...
impl ArrayWriter {
    ...
    fn new(name: &str) -> Self {
        Self {
            buffer: format!(r#"#[allow(dead_code)] pub const {}: &[u8] = &["#, name),
        }
    }
